    ...
    <div className="bg-neutral-900 rounded-2xl p-5 border border-neutral-800 space-y-3">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-white font-medium text-sm leading-tight">{nombre}</p>
            <span className={`text-xs font-medium ${tipo === 'personal' ? 'text-neutral-500' : 'text-indigo-400'}`}>
      ...
      {tipo === 'compartido' && codigo_invitacion && (
        <div className="flex items-center justify-between bg-neutral-800 rounded-xl px-3 py-2">
          <div>
            <p className="text-neutral-500 text-xs">Código de invitación</p>
            <p className="text-white font-mono font-bold tracking-widest text-sm">{codigo_invitacion}</p>
          </div>
          <button
      ...
      {tipo === 'compartido' && miembros.length > 0 && (
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap gap-1">
            {miembros.map(m => (
              <span key={m.user_id} className="text-xs bg-neutral-800 text-neutral-300 rounded-full px-2 py-0.5">
